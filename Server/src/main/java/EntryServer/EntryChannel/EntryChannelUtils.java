    ...
    public void communicateClient(JsonElement[] sortAPIList, int count, ChannelHandlerContext ctx) {
        ...
        if (count == 0) {
            ...
            if (clientRequest.getProtocol().equals("DataStream")) {
                ...
                System.out.println(("Here is non - 0 in client Function"));
        ...
        channel.pipeline().addAfter("Delimiter", UUID.randomUUID().toString() ,new SimpleChannelInboundHandler<ByteBuf>() {
            ...
            protected void channelRead0(ChannelHandlerContext ctx, ByteBuf msg) throws Exception {
                ...
                if (clientRequest.getBackPressureFlag() == 0) {
                    clientRequest.setBackPressureFlag(1);
                    System.out.println("waiting client back Pressure Again");
                    BackPressure.setBackPressureFlag(0);
                    System.out.println(BackPressure.backPressureFlag);
                    serverCtx.channel().writeAndFlush(responseToByteBuf(new ResponseFormat("flowControl")));
//                    msg.release();
                    return;
                } else {
